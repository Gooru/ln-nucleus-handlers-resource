...
class CreateResourceHandler implements DBHandler {
  ...
  public ExecutionResult<MessageResponse> checkSanity() {
    ...
    JsonObject errors = new DefaultPayloadValidator().validatePayload(context.request(),
        AJEntityOriginalResource.createFieldSelector(),
  ...
  public ExecutionResult<MessageResponse> validateRequest() {
    try {
      ...
      new DefaultOriginalResourceBuilder().build(resource, context.request(),
          AJEntityOriginalResource.getConverterRegistry());

      ExecutionResult<MessageResponse> result =
          ResourceUrlHelper.handleUrl(resource, context.request());
      if (result.hasFailed()) {
        return result;
      }
      ...
      if (resourceIdWithURLDuplicates != null && !resourceIdWithURLDuplicates.isEmpty()) {
        LOGGER.error(
            "validateRequest : Duplicate resource URL found. So cannot go ahead with creating new resource! "
                + "URL : {}",
            resource.getString(AJEntityResource.URL));
  ...
  private void autoPopulateFields() {
    ...
    TypeHelper.setPGObject(this.resource, AJEntityOriginalResource.CREATOR_ID,
        EntityConstants.UUID_TYPE, context.userId());
    TypeHelper.setPGObject(this.resource, AJEntityOriginalResource.MODIFIER_ID,
        EntityConstants.UUID_TYPE, context.userId());
  ...
  private static class DefaultOriginalResourceBuilder
      implements EntityBuilder<AJEntityOriginalResource> {
  }
