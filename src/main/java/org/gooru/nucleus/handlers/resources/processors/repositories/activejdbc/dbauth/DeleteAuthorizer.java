 ...
 */
final class DeleteAuthorizer implements Authorizer<ResourceHolder> {
    private final ProcessorContext context;
    private ResourceHolder holder;

    private static final Logger LOGGER = LoggerFactory.getLogger(DeleteAuthorizer.class);

    public DeleteAuthorizer(ProcessorContext context) {
    ...
    private ExecutionResult<MessageResponse> authorizeForResourceRefByCollection(String collection) {
        ...
        if (count >= 1) {
            LOGGER.debug("Auth check to delete resource '{}' successful based on collection: {}", context.resourceId(),
                collection);
    ...
    private ExecutionResult<MessageResponse> authorizeForResourceRefByCourse(String course) {
        final Long count =
            Base.count(AJEntityResource.TABLE_COURSE, AJEntityResource.AUTH_VIA_COURSE_FILTER, course, context.userId(),
                context.userId());
        if (count >= 1) {
            LOGGER.debug("Auth check to delete resource '{}' successful based on course: {}", context.resourceId(),
                course);
            return new ExecutionResult<>(null, ExecutionResult.ExecutionStatus.CONTINUE_PROCESSING);
        }
        return new ExecutionResult<>(
    ...
    private ExecutionResult<MessageResponse> authorizeForOriginalResource() {
        ...
        if (creator != null && creator.equalsIgnoreCase(context.userId())) {
            ...
            LOGGER.info("Delete Authorization failed for original resource '{}' for user '{}'", context.resourceId(),
                context.userId());
            return new ExecutionResult<>(
